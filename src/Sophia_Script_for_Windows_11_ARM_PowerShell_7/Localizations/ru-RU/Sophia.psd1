...
OneDriveWarning                           = Функция "WinPrtScrFolder -Desktop" будет применена только в случае, если пользователь настроил удаление OneDrive (или приложение уже удалено). Иначе ломается функционал резервного копирования для папок "Рабочий стол" и "Изображения" в OneDrive.
