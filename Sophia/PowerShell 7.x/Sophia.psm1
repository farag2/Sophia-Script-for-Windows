...
function TempFolder
{
	...
	switch ($PSCmdlet.ParameterSetName)
	{
		"SystemDrive"
		{
			...
			Get-ChildItem -Path $env:LOCALAPPDATA\Temp -Force | Where-Object -FilterScript {$_.Name -like "*remoteIpMoProxy*"} | ForEach-Object -Process {
...
<#
	...
	Set the "For All Users" checkbox checked by default
...
function UninstallUWPApps
{
	...
	<#
		.SYNOPSIS
		Add the classes from the WinRT.Runtime.dll and Microsoft.Windows.SDK.NET.dll libraries to the current session to be able to load localized UWP apps names
		...
		.NOTES
		CsWinRT v1.1.3
		Microsoft.Windows.SDK.NET 10.0.19041.14
	#>
	...
	function Get-AppxBundle
	{
		...
		$PackagesIds    = $PackageManager.FindPackages().AdditionalTypeData[[Collections.IEnumerable].TypeHandle]

		foreach ($AppxPackage in $AppxPackages)
		{
			$PackageId = $PackagesIds | Where-Object -FilterScript {$_.Id.Name -eq $AppxPackage.Name}
			...
			$AppxPackage | Add-Member -MemberType NoteProperty -Name DisplayName -Value $PackageId.DisplayName
			$AppxPackage | Add-Member -MemberType NoteProperty -Name FullName -Value $PackageId.Id.FullName
		}
